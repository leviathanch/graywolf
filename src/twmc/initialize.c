...
int cleanupReadCells()
{
	...
	netarrayG = (NETBOXPTR *) Ysafe_realloc( netarrayG, (numnetsG+1) * sizeof(NETBOXPTR) ) ;
	/* allocate and initialize terminal array */
	termarrayG = (PINBOXPTR *) Ysafe_calloc( (numpinsG + 1), sizeof( PINBOXPTR ) );
	...
	ASSERT( totalcellsG == endpadgrpsG+NUMPADMACROS, "cleanupReadCells", "cells don't add up" ) ;
	cellarrayG = (CELLBOXPTR *) Ysafe_realloc( cellarrayG,(totalcellsG+1)* sizeof(CELLBOXPTR) ) ;
...
void addCell( char *cellName, CELLTYPE cellType )
{
	...
	if( ++totalcellsG >= cellAllocS ){
		cellAllocS += EXPECTEDNUMCELLS ;
		cellarrayG = (CELLBOXPTR *) Ysafe_realloc( cellarrayG, cellAllocS*sizeof(CELLBOXPTR) );
	}
	ptrS = cellarrayG[totalcellsG] = (CELLBOXPTR) Ysafe_malloc( sizeof(CELLBOX) ) ;
	...
	*data = totalcellsG ;
	...
	if( cellType == SOFTCELLTYPE || cellType == CUSTOMCELLTYPE || cellType == STDCELLTYPE ){
		for( i = 0 ; i < 8 ; i++ ) {
	...
	if( cellType == SOFTCELLTYPE ){
		...
		if( cellType == PADGROUPTYPE ){
			...
			pptrS->children = (int *) Ysafe_malloc((childAllocS)*sizeof(int));
		...
		curGroupS =ptrS->group = (GROUPBOXPTR) Ysafe_malloc( sizeof(GROUPBOX) ) ;
...
void endCell()
{
	...
	if( curCellTypeS == CUSTOMCELLTYPE || curCellTypeS == SOFTCELLTYPE || curCellTypeS == STDCELLTYPE ){
	...
	if( curCellTypeS == SOFTCELLTYPE || curCellTypeS == STDCELLTYPE ){
		...
		pptrS->children = (int *) Ysafe_realloc( pptrS->children,(numchildrenS+1) * sizeof(int));
...
/* ***************************************************************** */
static int findsidestr( char *side, BOOL direction )
{
	if( direction == TRUE ){
		/* BT case */
...
void fixCell( int fixedType, int xloc, char *lorR, int yloc, char *borT, int xloc2, char *lorR2, int yloc2, char *borT2 ) /* valid types - neighborhood. point, group */
{
	...
	if( fixedType == GROUPFLAG || fixedType == FIXEDGROUPFLAG ) {
		...
		fixptr =ptrS->fixed = (FIXEDBOXPTR) Ysafe_malloc( sizeof(FIXEDBOX) ) ;
	}
...
void processCorners( int numcorners )
{
	...
	if( cornerCountS != numcorners ){
		sprintf( YmsgG, "Incorrect number %d of corners for cell: %s \n", cornerCountS, curCellNameS ) ;
	...
	if( curCellTypeS == SOFTCELLTYPE || curCellTypeS == CUSTOMCELLTYPE || curCellTypeS == STDCELLTYPE ) {
		...
		for( k = 1 ; k <= numcorners ; k++ ) {
			if( k < numcorners ) {
				...
				pSideArrayS[k].length = ABS(cornerArrayS[1].x - cornerArrayS[k].x) ;
				pSideArrayS[k].vertical = 0 ;
				pSideArrayS[k].pincount = 0 ;
				pSideArrayS[k].position = cornerArrayS[k].y ;
			}
	...
	ptrS->bounBox = (BOUNBOXPTR *) Ysafe_malloc( 9 * sizeof( BOUNBOXPTR ) ) ;
	for( k=0;k<=8;k++){
	...
	while( busterptr = Ybuster() ){
		...
		if( tile ) {
			...
			tile = ptrS->tiles = 	(TILEBOXPTR) Ysafe_malloc( sizeof( TILEBOX ));
		}
		tile->next = NULL ;
		ptrS->numtiles++ ;
	...
	ptrS->orig_aspect = ptrS->aspect = (double)(maxyS - minyS) / (double)(maxxS - minxS);
...
void addCorner( int xpos, int ypos )
{
	if( ++cornerCountS >= tileptAllocS ){
		...
		cornerArrayS = ( YBUSTBOXPTR ) Ysafe_realloc( cornerArrayS, tileptAllocS * sizeof( YBUSTBOX ) );
		pSideArrayS  = (PSIDEBOX *) Ysafe_realloc( pSideArrayS, tileptAllocS * sizeof( PSIDEBOX ) ) ;
	}
	if( scale_dataG ){
		if( portFlagS ){
...
PINBOXPTR addPinAndNet( char *pinName, char *signal )
{
	...
	data = Yhash_search( netTableS, signal, NULL, FIND );
	if(data){
		netx = *data ;
		notInTable = FALSE ;
	} else {
...
void addPin( char *pinName, char *signal, int layer, int pinType )
{
	...
	if( pinS ){
		...
		pinS->txpos_orig = (int*) Ysafe_realloc( pinS->txpos_orig,(cellinstanceS+1)*sizeof(int));
		pinS->typos_orig = (int*) Ysafe_realloc( pinS->typos_orig,(cellinstanceS+1)*sizeof(int));
...
void add_soft_array()
{
	...
	sarray = ptrS->softpins = (PINBOXPTR *) Ysafe_malloc( (ptrS->numpins+1) * sizeof(PINBOXPTR) ) ;
	i = 0 ;
...
void start_pin_group( char *pingroup, BOOL permute )
{
	...
	if( cellinstanceS == 0 ) {
		...
		sarray = ptrS->softpins = (PINBOXPTR *) Ysafe_realloc( ptrS->softpins, (curpingroup+1) * sizeof(PINBOXPTR) ) ;
		sarray[HOWMANY] = (PINBOXPTR) curpingroup ;

		/* allocate space for pingroup */
		pingroupS = sarray[curpingroup] = (PINBOXPTR) Ysafe_calloc( 1, sizeof(PINBOX) );
	...
	/*  now allocate space for this instance */
	pingroupS->soft_inst[cellinstanceS] = pingroupS->softinfo = (SOFTBOXPTR) Ysafe_calloc( 1, sizeof(SOFTBOX)) ;
...
/* add this pad to the current pad group */
void add2padgroup( char *padName, BOOL ordered )  /* ordered flag is true if pad is ordered in padgroup */
{
	...
	for (i = numcellsG + 1; i <= endofpads; ++i) {
		if (strcmp(padName, cellarrayG[i]->cname) == STRINGEQ) {
			if (cellarrayG[i]->padptr->hierarchy == LEAF) {
				sprintf(YmsgG, "pad %s was included in more than 1 pad group\n", cellarrayG[i]->cname);
...
void add_analog( int numcorners )
{
	...
	if( cellinstanceS == 0 ){
		...
		pinS->ainst = (ANALOGPTR *) Ysafe_realloc( pinS->ainst, instAllocS * sizeof(ANALOGPTR) );
	}
