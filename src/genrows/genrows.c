	...
	for( tile1 = tile_listG ; tile1 ; tile1 = tile1->next ) {
		for( tile2 = tile1->next ; tile2 ; tile2 = tile2->next ) {
			if( !(tile1->lly >= tile2->ury || tile2->lly >= tile1->ury)){
				if( tile1->actual_row_height!=tile2->actual_row_height || tile1->channel_separation != tile2->channel_separation){
					printf("\nERROR:\n") ;
...
void print_blk_file() 
{
		...
		if( segment == NULL ) {
			if( feeds_per_segG ){
				...
				if( r > rowptr->llx ){
					...
					fprintf(fpb,"%d %d    %d %d", rowptr->llx, rowptr->lly, r, rowptr->ury);
			...
			fprintf(fpb,"%d %d    %d %d", rowptr->llx, rowptr->lly, rowptr->urx, rowptr->ury);
			if( rowptr->class != 1 ){
			...
			if( feeds_per_segG && r > last_llx ){
				fprintf(fpb,"%d %d    %d %d ", llx, rowptr->lly, r, rowptr->ury ) ;
			} else {
				fprintf(fpb,"%d %d    %d %d ", llx, rowptr->lly, urx, rowptr->ury ) ;
			}
			...
			while( TRUE ) {
				...
				segment = rowptr->next_segment ;
				while( segment ) {
					if( segment->llx > last_llx && segment->llx < next_llx ) {
				...
				if( save_segment ) {
					...
					if( feeds_per_segG && r > last_urx ){
						fprintf(fpb," except %d %d", last_urx , r ) ;
					} else if( next_llx > last_urx ){
						fprintf(fpb," except %d %d", last_urx , next_llx ) ;
					}
					last_llx = next_llx ;
					next_llx = INT_MAX ;
	...
	for( tileptr=tile_listG;tileptr;tileptr=tileptr->next ){
		if( tileptr->numrows > 0 ){
			fprintf(fpb,"   %d %d   %d %d\n", tileptr->llx, tileptr->lly, tileptr->urx, tileptr->ury ) ;
		}
	}
...
void print_tiles() 
{
	...
	for( tileptr=tile_listG;tileptr;tileptr=tileptr->next ){
		i++;
		printf("TILE:%4d\t(%4d %4d)   (%4d %4d)\n", i, tileptr->llx, tileptr->lly, tileptr->urx, tileptr->ury) ;
	}
	printf("\n");
	return ;
} /* end print_tiles */
...
BOOL find_tile( int *llx , int *lly , int *urx , int *ury )
{
	...
	*ury = top_edge ;
	...
	if( !found ) {
		last_vertexS = last_vertexS->next = (VERTEX_BOX *) Ysafe_malloc(sizeof(VERTEX_BOX));
		last_vertexS->x = *llx ;
		last_vertexS->y = *ury ;
...
process_vertices()
{
	...
	for( check_vertex=vertex_listS;check_vertex;check_vertex=check_vertex->next ){
		if( check_vertex->class == MAYBE_END ) {
			...
			check_macro = check_vertex->macro ;
			for( vertex = vertex_listS ;vertex ; vertex = vertex->next ){
				if( vertex->macro != check_macro && vertex->next && vertex->macro == vertex->next->macro && vertex->x == vertex->next->x && vertex->x == check_vertex->x ) {
					if( vertex->y > vertex->next->y ) {
						if( check_vertex->y > vertex->next->y && check_vertex->y < vertex->y ) {
...
void makerows()
{
	...
	while( cx2S - cx1S + row_length_so_far <= total_row_lengthS ) {
		if( last_rowptrS == NULL ) {
			...
			rowptr = last_rowptrS->next_row = (ROW_BOX *) Ysafe_malloc( sizeof( ROW_BOX ) ) ;
			last_rowptrS = rowptr ;
		}
		top_rows = TRUE ;
...
int set_minimum_length( int length )
{
	TILE_BOX *tileptr ;      /* current tile */
